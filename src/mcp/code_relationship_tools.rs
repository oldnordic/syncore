...
impl CodeRelationshipTools {
    ...
    {
        let wrapped = Arc::new(move |name: String, data: Value| callback(&name, &data));
    ...
    pub async fn handle_code_relationship_query(&self, params: Value) -> Result<Value> {
        ...
        match query_type {
            ...
            "implementors" => {
                let trait_name = params["trait_name"].as_str().ok_or_else(|| {
                    anyhow!("Missing trait_name parameter for implementors query")
                })?;
    ...
    async fn get_function_calls(&self, function: &str) -> Result<Vec<String>> {
        ...
        let mut stmt = db.prepare("SELECT DISTINCT callee FROM code_calls WHERE caller = ?1")?;
        let calls = stmt
            .query_map([function], |row| row.get::<_, String>(0))?
    ...
    async fn get_trait_implementors(&self, trait_name: &str) -> Result<Vec<String>> {
        let db = self.store.db.lock().await;
        let mut stmt =
            db.prepare("SELECT DISTINCT struct_name FROM code_impls WHERE trait_name = ?1")?;
        let implementors = stmt
            .query_map([trait_name], |row| row.get::<_, String>(0))?
            .collect::<std::result::Result<Vec<String>, _>>()?;
        Ok(implementors)
    }
