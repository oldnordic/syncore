...
impl RealExecutor {
    ...
    pub async fn execute_real_tool_async(
        ...
        match tool_name {
            ...
            // ================================================================
            "vector_insert" => {
                executors::vector_executor::execute_vector_insert(&self.state, params, dry_run).await
            }

            "vector_search" => {
                executors::vector_executor::execute_vector_search(&self.state, params, dry_run).await
            }
            ...
            "intellitask_list" => {
                executors::task_executor::execute_intellitask_list(&self.state, params, dry_run).await
            }

            "intellitask_get" => {
                executors::task_executor::execute_intellitask_get(&self.state, params, dry_run).await
            }

            "intellitask_update_status" => {
                executors::task_executor::execute_intellitask_update_status(&self.state, params, dry_run).await
            }

            "intellitask_next_ready" => {
                executors::task_executor::execute_intellitask_next_ready(&self.state, params, dry_run).await
            }

            "intellitask_get_subtasks" => {
                executors::task_executor::execute_intellitask_get_subtasks(&self.state, params, dry_run).await
            }

            "intellitask_subtask_stats" => {
                executors::task_executor::execute_intellitask_subtask_stats(&self.state, params, dry_run).await
            }

            "intellitask_task_statistics" => {
                executors::task_executor::execute_intellitask_task_statistics(&self.state, params, dry_run).await
            }

            "intellitask_prd_statistics" => {
                executors::task_executor::execute_intellitask_prd_statistics(&self.state, params, dry_run).await
            }
            ...
            // ================================================================
            "parser_analyze" => {
                executors::code_parser_executor::execute_parser_analyze(&self.state, params, dry_run).await
            }

            "parser_search" => {
                executors::code_parser_executor::execute_parser_search(&self.state, params, dry_run).await
            }

            "code_index" => {
                executors::code_parser_executor::execute_code_index(&self.state, params, dry_run).await
            }

            "code_index_directory" => {
                executors::code_parser_executor::execute_code_index_directory(&self.state, params, dry_run).await
            }

            "code_search" => {
                executors::code_parser_executor::execute_code_search(&self.state, params, dry_run).await
            }
            ...
            // ================================================================
            "document_index" => {
                executors::document_executor::execute_document_index(&self.state, params, dry_run).await
            }

            "document_search" => {
                executors::document_executor::execute_document_search(&self.state, params, dry_run).await
            }
            ...
            // ================================================================
            "graph_query" => {
                executors::graph_executor::execute_graph_query(&self.state, params, dry_run, tool_name).await
            }

            "graph_insert" => {
                executors::graph_executor::execute_graph_insert(&self.state, params, dry_run, tool_name).await
            }

            "graph_relate" => {
                executors::graph_executor::execute_graph_relate(&self.state, params, dry_run, tool_name).await
            }

            "raggraph_query" => {
                executors::graph_executor::execute_raggraph_query(&self.state, params, dry_run, tool_name).await
            }

            "raggraph_multihop" => {
                executors::graph_executor::execute_raggraph_multihop(&self.state, params, dry_run, tool_name).await
            }
            ...
            "code_graph_sync_neo4j" => {
                executors::graph_executor::execute_code_graph_sync_neo4j(&self.state, params, dry_run, tool_name).await
            }

            "code_graph_enrich_temporal" => {
                executors::graph_executor::execute_code_graph_enrich_temporal(&self.state, params, dry_run, tool_name).await
            }

            "code_graph_fusion_query" => {
                executors::graph_executor::execute_code_graph_fusion_query(&self.state, params, dry_run, tool_name).await
            }
            ...
            "agent_register" => {
                executors::agent_executor::execute_agent_register(&self.state, params, dry_run).await
            }
            ...
            // ================================================================
            "mapping_record" => {
                executors::mapping_executor::execute_mapping_record(&self.state, params, dry_run).await
            }
            ...
            "mapping_search" => {
                executors::mapping_executor::execute_mapping_search(&self.state, params, dry_run).await
            }

            "mapping_deps" => {
                executors::mapping_executor::execute_mapping_deps(&self.state, params, dry_run).await
            }
            ...
            // ================================================================
            "application_record" => {
                let result = executors::application_executor::execute_application_record(&self.state, params).await;
                Ok(self.route_through_suite(result))
            }

            "application_get" => {
                let result = executors::application_executor::execute_application_get(&self.state, params).await;
                Ok(self.route_through_suite(result))
            }

            "application_history" => {
                let result = executors::application_executor::execute_application_history(&self.state, params).await;
                Ok(self.route_through_suite(result))
            }

            "application_search" => {
                let result = executors::application_executor::execute_application_search(&self.state, params).await;
                Ok(self.route_through_suite(result))
            }
