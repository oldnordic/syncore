...
fn test_router_maps_memory_store_to_correct_handler() {
    ...
    conn.execute("INSERT INTO memory (k, v, ts) VALUES (?1, ?2, datetime('now'))", [key, value]).unwrap();
...
fn test_router_maps_task_next_to_correct_handler() {
    ...
    let task1_id = syncore::tasks::add_task(&conn, "High priority", "Description 1", 1, None).unwrap();
    let task2_id = syncore::tasks::add_task(&conn, "Low priority", "Description 2", 3, None).unwrap();
...
fn test_router_maps_task_link_to_correct_handler() {
    ...
    // Create test tasks
    let task1_id = syncore::tasks::add_task(&conn, "Prerequisite", "Must complete first", 1, None).unwrap();
    let task2_id = syncore::tasks::add_task(&conn, "Dependent", "Depends on prerequisite", 2, None).unwrap();
...
fn test_router_maps_cognitive_step_to_correct_handler() {
    ...
    // Create a task first
    let task_id = syncore::tasks::add_task(&conn, "Test task", "For cognitive steps", 1, None).unwrap();

    // Call the actual cognitive_db function
    let step1_id = syncore::cognitive_db::store_step(&conn, Some(task_id), "Think", "Initial thinking process", "{}").unwrap();
    let step2_id = syncore::cognitive_db::store_step(&conn, Some(task_id), "Decide", "Made decision to proceed", "{\"decision\": \"action\"}").unwrap();
...
fn test_router_maps_vector_operations_to_correct_handler() {
    ...
    // Test insert_text
    let result = syncore::vector::insert_text(&mut vector_store, 1, Some(100), "apple orange banana", "note");
    assert!(result.is_ok(), "vector::insert_text should succeed");

    // Test search
    let hits = syncore::vector::search(&vector_store, "apple", 5, syncore::vector::SearchScope::Global).unwrap();
    ...
    let result2 = syncore::vector::insert_text(&mut vector_store, 2, Some(200), "car truck bicycle", "note");
    assert!(result2.is_ok(), "vector::insert_text should succeed");

    let task_hits = syncore::vector::search(&vector_store, "apple", 5, syncore::vector::SearchScope::Task(100)).unwrap();
    ...
    let different_task_hits = syncore::vector::search(&vector_store, "apple", 5, syncore::vector::SearchScope::Task(200)).unwrap();

    // Verify that task-scoped search returns fewer or equal results than global search
    let global_hits = syncore::vector::search(&vector_store, "apple", 5, syncore::vector::SearchScope::Global).unwrap();
    assert!(different_task_hits.len() <= global_hits.len(), "Task-scoped search should not return more results than global search");

    // Verify that the task-scoped results all have the correct task_id
    for hit in &different_task_hits {
        assert_eq!(hit.task_id, Some(200), "Task-scoped search should only return results from the specified task");
    }
...
fn simulate_mcp_request(tool_name: &str, args: &HashMap<String, Value>, conn: &rusqlite::Connection, vector_store: &mut vector::VectorStore) -> Result<Value, String> {
    match tool_name {
        "memory.store" => {
        ...
        "task.create" => {
            ...
            Ok(create_success_response(json!({"task_created": format!("{} - {} (ID: {})", goal, description, task_id)})))
        },
        ...
        "cognitive.step" => {
            ...
            Ok(create_success_response(json!({"step_stored": format!("{}: {} (ID: {})", state, content, step_id)})))
        },
        ...
        "vector.insert" => {
            ...
            Ok(create_success_response(json!({"vector_inserted": format!("{} in task {}", text, task_id.unwrap_or(0))})))
        },
