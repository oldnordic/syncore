...
fn test_large_dataset_search_performance() {
    ...
    // Insert 5000 vectors
    for i in 0..5000 {
        let text = format!("document {} about topic {} with content {}", i, i % 100, i % 10);
    ...
    let results = store.search("document topic", 10, SearchScope::Global).expect("Search failed");
...
fn test_no_linear_scans() {
    ...
    // Insert 100 vectors
    for i in 0..100 {
        store.insert_text(i as i64, None, &format!("doc {}", i), "test").expect("Insert failed");
    }

    // First search
    let start1 = Instant::now();
    let _results1 = store.search("doc", 10, SearchScope::Global).expect("Search failed");
    let duration1 = start1.elapsed();

    // Second search - should NOT rescan all vectors
    let start2 = Instant::now();
    let _results2 = store.search("doc", 10, SearchScope::Global).expect("Search failed");
...
fn test_query_cache_repeated_searches() {
    ...
    let start1 = Instant::now();
    let results1 = store.search(query, 5, SearchScope::Global).expect("Search failed");
    let duration1 = start1.elapsed();

    // Second search (warm cache)
    let start2 = Instant::now();
    let results2 = store.search(query, 5, SearchScope::Global).expect("Search failed");
    let duration2 = start2.elapsed();
...
fn test_insert_latency_reasonable() {
    ...
    assert!(avg_us < 1000, "Average insert latency {}µs too high (expected < 1000µs)", avg_us);

    // Later inserts shouldn't be much slower (no O(n²) behavior)
    let early_avg: u128 = insert_times[0..10].iter().map(|d| d.as_micros()).sum::<u128>() / 10;
    let late_avg: u128 = insert_times[90..100].iter().map(|d| d.as_micros()).sum::<u128>() / 10;
...
fn test_search_latency_reasonable() {
    ...
    for _ in 0..20 {
        let start = Instant::now();
        let _results = store.search("document", 10, SearchScope::Global).expect("Search failed");
    ...
    assert!(avg_us < 10000, "Average search latency {}µs too high (expected < 10000µs)", avg_us);
...
fn test_memory_efficient_large_dataset() {
    ...
    let results = store.search("doc", 10, SearchScope::Global).expect("Search failed");
...
fn test_batch_insert_performance() {
    ...
    assert!(duration.as_secs() < 1, "Batch insert took {:?} - too slow", duration);
}
