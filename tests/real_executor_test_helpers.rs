...
pub fn unwrap_data(v: &Value) -> &Value {
    assert_eq!(
        v.get("ok"),
        Some(&json!(true)),
        "Expected ok=true in envelope: {:?}",
        v
    );
    v.get("data")
        .expect("Missing data field in success envelope")
}
...
pub fn unwrap_error(v: &Value) -> &Value {
    assert_eq!(
        v.get("ok"),
        Some(&json!(false)),
        "Expected ok=false in envelope: {:?}",
        v
    );
    v.get("error")
        .expect("Missing error field in error envelope")
}
