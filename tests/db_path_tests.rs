...
fn test_default_path_resolves_to_project_root() {
    ...
    assert_ne!(path, "syncore_code_graph.db", "Path must not be relative string");
}
...
fn test_path_absolute_from_any_cwd() {
    ...
    // MUST be absolute
    assert!(path_buf.is_absolute(), "Path must be absolute even without env var, got: {}", path);
}
...
fn test_main_db_path_absolute() {
    ...
    // Must be absolute
    assert!(path_buf.is_absolute(), "Main DB path must be absolute, got: {}", path);

    // Must end with syncore.db
    assert!(path.ends_with("syncore.db"), "Path must end with syncore.db, got: {}", path);
