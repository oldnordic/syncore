...
async fn test_sqlite_fast_path_parity() -> Result<()> {
    ...
    for &entity_id in &entity_ids {
        ...
        for (i, (neighbor1, neighbor2)) in
            sqlite_neighbors_1.iter().zip(sqlite_neighbors_2.iter()).enumerate()
        {
            assert_eq!(
...
async fn test_cache_invalidation_parity() -> Result<()> {
    ...
    for (i, (initial, updated)) in
        sqlite_neighbor_counts.iter().zip(sqlite_updated_counts.iter()).enumerate()
    {
    ...
    for (i, (initial, updated)) in
        neo4j_neighbor_counts.iter().zip(neo4j_updated_counts.iter()).enumerate()
    {
        if i < entity_ids.len() / 2 || i >= entity_ids.len() - entity_ids.len() / 2 {
            // These entities should have new relationships
            assert!(
...
async fn test_cache_performance_consistency() -> Result<()> {
    ...
    for iteration in 1..=iterations {
        ...
        if iteration > 1 {
            ...
            assert_eq!(
                sqlite_total,
                500, // 100 entities * 5 relationships each
...
async fn test_deletion_cache_behavior() -> Result<()> {
    ...
    for &entity_id in &entity_ids {
        if entities_to_delete.contains(&entity_id) {
            ...
            assert!(
                sqlite_neighbors.is_empty(),
                "Deleted entity should have no neighbors in SQLite"
            );
        } else {
...
async fn test_file_operations_cache_behavior() -> Result<()> {
    ...
    for iteration in 1..=5 {
        ...
        assert_eq!(neo4j_file_entities.len(), 10, "Should have 10 entities in file");

        // Verify ordering consistency
        for (i, (neo4j_entity, sqlite_entity)) in
            neo4j_file_entities.iter().zip(sqlite_file_entities.iter()).enumerate()
        {
            assert_eq!(
...
async fn test_concurrent_cache_behavior() -> Result<()> {
    ...
    for (neo4j_total, sqlite_total, thread_id) in results {
        assert_eq!(neo4j_total, sqlite_total, "Thread {} got inconsistent totals", thread_id);
        assert_eq!(
            neo4j_total,
            19, // 20 entities with 19 relationships in a chain
