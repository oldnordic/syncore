...
async fn faiss_pool_provides_index_instances() -> Result<()> {
    let pool = FaissPool::new("tests/data/faiss_test.index", 4);
    let conn = pool.pool.get().await.map_err(|e| anyhow::anyhow!("Pool error: {:?}", e))?;
...
async fn faiss_pool_recycles_connections() -> Result<()> {
    ...
    // Get two connections
    let conn1 = pool.pool.get().await.map_err(|e| anyhow::anyhow!("Pool error: {:?}", e))?;
    let conn2 = pool.pool.get().await.map_err(|e| anyhow::anyhow!("Pool error: {:?}", e))?;
    ...
    let conn3 = pool.pool.get().await.map_err(|e| anyhow::anyhow!("Pool error: {:?}", e))?;
