...
use syncore::{tasks, memory, vector, router};
use serde_json::json;
...
fn test_vector_insert_tool() {
    ...
    let vector_store = std::sync::Arc::new(std::sync::Mutex::new(vector::VectorStore::new(embeddings)));
    
    let state = router::SynCoreState::new(memory, tasks, vector_store);
    
    // Test vector.insert tool
    let args = rmp_serde::to_vec(&(1i64, Some(2i64), "test document".to_string(), "note".to_string())).unwrap();
...
fn test_graph_link_tool() {
    ...
    let task1_id = state.tasks.with_db(|db| {
        tasks::add_task(db, "Task 1", "Description 1", 1, None)
    }).unwrap();
    
    let task2_id = state.tasks.with_db(|db| {
        tasks::add_task(db, "Task 2", "Description 2", 2, None)
    }).unwrap();
...
fn test_graph_query_tool() {
    ...
    let task3_id = state.tasks.with_db(|db| {
        tasks::add_task(db, "Task 3", "Description 3", 3, None)
    }).unwrap();
