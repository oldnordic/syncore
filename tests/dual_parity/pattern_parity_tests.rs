...
async fn test_directionality_parity() -> Result<()> {
    ...
    assert_eq!(
        neo4j_outgoing.len(),
        sqlite_outgoing.len(),
        "Outgoing relationship count mismatch"
    );
    ...
    assert_eq!(
        neo4j_incoming.len(),
        sqlite_incoming.len(),
        "Incoming relationship count mismatch"
    );
...
async fn test_property_filtering_parity() -> Result<()> {
    ...
    for i in 1..=10 {
        let props = NodeProperties {
            ...
            docstring: if i % 2 == 0 { Some(format!("Documentation for {}", i)) } else { None },
    ...
    assert_eq!(
        entity_5_neo4j.len(),
        entity_5_sqlite.len(),
        "Name-based search count mismatch"
    );
    
    if !entity_5_neo4j.is_empty() && !entity_5_sqlite.is_empty() {
        assert_eq!(
            entity_5_neo4j[0].id,
            entity_5_sqlite[0].id,
            "Entity 5 ID mismatch"
        );
        assert_eq!(
            entity_5_neo4j[0].name,
            entity_5_sqlite[0].name,
            "Entity 5 name mismatch"
        );
    }
