...
async fn test_neighbor_ordering_consistency() -> Result<()> {
    ...
    setup.sqlite.upsert_entity(NodeLabel::Function, central_props).await?;
    ...
    for i in 2..=12 {
        ...
        setup.sqlite.upsert_entity(NodeLabel::Function, neighbor_props).await?;
        neighbor_ids.push(i);
    }
    ...
    for iteration in 1..=5 {
        ...
        assert!(
            compare_ordered_results(&neo4j_neighbors, &sqlite_neighbors, &format!("iteration {}", iteration))?,
        ...
        if iteration > 1 {
            ...
            for (i, (neo4j_entity, sqlite_entity)) in neo4j_neighbors.iter().zip(sqlite_neighbors.iter()).enumerate() {
                assert_eq!(
...
async fn test_file_entity_ordering_consistency() -> Result<()> {
    ...
    for iteration in 1..=3 {
        ...
        assert!(
            compare_ordered_results(&neo4j_file_entities, &sqlite_file_entities, &format!("file iteration {}", iteration))?,
        ...
        for i in 1..neo4j_file_entities.len() {
            ...
            assert!(
                ...
                prev.name, prev.start_line.unwrap_or(0),
                curr.name, curr.start_line.unwrap_or(0)
            );
        }
...
async fn test_entity_type_ordering_consistency() -> Result<()> {
    ...
    for iteration in 1..=3 {
        ...
        assert!(
            compare_ordered_results(&neo4j_functions, &sqlite_functions, &format!("functions iteration {}", iteration))?,
        ...
        assert!(
            compare_ordered_results(&neo4j_structs, &sqlite_structs, &format!("structs iteration {}", iteration))?,
...
async fn test_name_search_ordering_consistency() -> Result<()> {
    ...
    for iteration in 1..=3 {
        ...
        assert!(
            compare_ordered_results(&neo4j_results, &sqlite_results, &format!("name search iteration {}", iteration))?,
...
async fn test_orphan_detection_ordering_consistency() -> Result<()> {
    ...
    setup.sqlite.upsert_entity(NodeLabel::Function, connected_props).await?;
    ...
    for iteration in 1..=3 {
        ...
        assert!(
            compare_ordered_results(&neo4j_orphans, &sqlite_orphans, &format!("orphan iteration {}", iteration))?,
            "Orphan detection ordering mismatch in iteration {}",
            iteration
        );
        
        // Should have exactly 10 orphans (not the connected one)
        ...
        for backend_results in [&neo4j_orphans, &sqlite_orphans] {
            for i in 1..backend_results.len() {
                ...
                if prev.name == curr.name {
                    assert!(
                        prev.id < curr.id,
                        "Orphans with same name should be ordered by ID"
                    );
                } else {
                    assert!(
                        prev.name < curr.name,
                        "Orphans should be ordered by name"
                    );
                }
            }
        }
...
async fn test_complex_relationship_ordering() -> Result<()> {
    ...
    setup.sqlite.upsert_entity(NodeLabel::Function, hub_props).await?;
    ...
    for iteration in 1..=3 {
        ...
        assert!(
            compare_ordered_results(&neo4j_neighbors, &sqlite_neighbors, &format!("complex iteration {}", iteration))?,
