...
fn test_tasks_add_and_retrieve_task() {
    ...
    // Add a task
    let task_id = tasks.add_task("Optimize SIMD kernel", "Created via test", 8, None).unwrap();
...
fn test_tasks_priority_ordering() {
    ...
    let high_id =
        tasks.add_task("High priority task", "High priority description", 9, None).unwrap();
    let _medium_id =
        tasks.add_task("Medium priority task", "Medium priority description", 5, None).unwrap();
